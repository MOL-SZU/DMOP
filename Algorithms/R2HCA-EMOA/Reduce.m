function [Population,worst,tensor] = Reduce(Problem,Population,W,worst,tensor,r,row)
...
%--------------------------------------------------------------------------  
    ...
    % Dynamic stage switches may transiently leave objective dimensions stale.
    % Align objectives to current Problem.M before tensor/update calculations.
    if isempty(PopObj)
        PopObj = zeros(0,Problem.M);
    elseif size(PopObj,2) > Problem.M
        PopObj = PopObj(:,1:Problem.M);
    elseif size(PopObj,2) < Problem.M
        PopObj = [PopObj,zeros(size(PopObj,1),Problem.M-size(PopObj,2))];
    end
    fmax   = max(PopObj,[],1);
    fmin   = min(PopObj,[],1);
    span   = max(fmax-fmin,1e-12);
    PopObj  = (PopObj-repmat(fmin,size(PopObj,1),1))./repmat(span,size(PopObj,1),1);
    tensor = UpdateUtilityTensor(Problem,PopObj,worst,r,tensor,W,row);
    ...
    if N > 1 && N < Problem.N+1
        ...
        deltaS = CalR2HVC(Problem.M,tensor1);
    elseif N==1
        deltaS = 0;
    else
        deltaS = CalR2HVC(Problem.M,tensor);
    end
